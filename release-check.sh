...
#
# 0.60.1 and 0.60.2 were tagged on two branches off 0.60.0 and neither was
# merged back, so the higher version shipped WITHOUT the lower one's fix and
# two fleets had to pin the lower version to keep it. Nothing was red: both
# tags built, both published, and the regression only showed up in
# production. This gate refuses that shape at tag time.
#
